...
public class DAO {
	...
	private Connection getConnection() throws Exception {
		...
		return ds.getConnection();
			...
			public int getQnAArticleCount(String sqry) throws Exception {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				...
				try {
					conn = getConnection();
					pstmt = conn.prepareStatement("select count(*) from QnA " + sqry);	//	public int getArticleCount(String sqry) throws Exception {
					rs = pstmt.executeQuery();
					if (rs.next()) {
						x= rs.getInt(1);
					}
			...
			public List getQnAArticles(int start, int end, String sqry) throws Exception {
				Connection conn = null;
				PreparedStatement pstmt = null;
				ResultSet rs = null;
				...
				try {
					...
					conn = getConnection();
					col = "q_num, q_writer, q_subject,q_password2, q_reg_date, q_content,q_readcount ,q_re_step , q_re_level , q_ref ";
					pstmt = conn.prepareStatement("select "+col+" from ( select "+col+", rownum r"+" from ( select "+col+" from QnA "+sqry+" order by q_reg_date) ) where r between ? and ?");
...
public List getStyleNewsArticles(int start, int end, String sqry) throws Exception {
	...
	try {
		...
		conn = getConnection();
		col = "s_num, s_writer, s_subject,s_password, s_reg_date, s_content,s_readcount";
		  pstmt = conn.prepareStatement("select "+col+" from ( select "+col+", rownum r"+" from ( select "+col+" from StyleNews "+sqry+" order by s_reg_date desc) ) where r between ? and ?");
		pstmt.setInt(1, start);
		pstmt.setInt(2, end);
