...
<%
    ...
    if (count > 0) {
    	reservationList = dbPro.getReservations(startRow, endRow);
    }
...
<tr>
    <td align="right" >
    <%if(session.getAttribute("r_nickname")!=null) {%>
...
%>
   <tr height="30">
    ...
    <td align="center"  width="150"><%= sdf.format(reservation.getR_reg_date())%></td>
  </tr>
     <%}%>
</table>
<%}%>
...
<%
    if (count > 0) {
        ...
        if (startPage > 10) {    %>
        <a href="AdminReserveList.jsp?pageNum=<%= startPage - 10 %>">[이전]</a>
<%      }
        for (int i = startPage ; i <= endPage ; i++) {  %>
        <a href="AdminReserveList.jsp?pageNum=<%= i %>">[<%= i %>]</a>
<%
        }
        if (endPage < pageCount) {  %>
        <a href="AdminReserveList.jsp?pageNum=<%= startPage + 10 %>">[다음]</a>
<%
        }
    }
%>
